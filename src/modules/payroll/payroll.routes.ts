...
export function payrollRoutes(deps: PayrollRouteDeps): RouteDefinition[] {
  ...
  return [
    ...
    { method: 'POST', path: '/api/v1/mosques/:mosqueId/payroll/runs', permission: 'TENANT_SCOPED',
      ...
      docs: { summary: 'Generate a payroll run for a period', body: createPayrollRunSchema, response: payrollRunResponseSchema } },
    ...
    { method: 'POST', path: '/api/v1/mosques/:mosqueId/payroll/runs/:runId/post', permission: 'TENANT_SCOPED',
      middleware: [requireAuth, tenantGuard, requireRunner, createRequireIdempotency(deps.idempotency)],
      handler: pc.postRun,
      docs: { summary: 'Post a payroll run to the expense ledger', response: payrollRunResponseSchema } },
  ];
}
